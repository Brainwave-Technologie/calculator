// models/VerismaDailyAllocation.js - Complete Verisma Daily Allocation with Edit/Delete workflow
...
const VerismaDailyAllocationSchema = new mongoose.Schema({
  ...
  // Process (from Excel - typically same as project_name)
  process: { type: String, trim: true, default: '' },
  
  // Facility (Free text field from Excel)
  ...
  // Note: For Verisma, duplicate Request IDs become "Duplicate" (not "Follow up" like MRO)
  request_type: { 
    type: String, 
    enum: ['', 'New Request', 'Duplicate', 'Key'],
    required: true
  },
  
  // Requestor Type for Verisma (from Excel)
  requestor_type: {
    type: String,
    enum: [
      '',
  ...
  // Bronx Care Processing Time (from Excel)
  bronx_care_processing_time: { type: String, trim: true, default: '' },
...
// ============ INDEXES ============
VerismaDailyAllocationSchema.index({ resource_email: 1, allocation_date: 1 });
VerismaDailyAllocationSchema.index({ resource_email: 1, month: 1, year: 1 });
VerismaDailyAllocationSchema.index({ subproject_key: 1, month: 1, year: 1 });
VerismaDailyAllocationSchema.index({ request_id: 1, client_name: 1 });
VerismaDailyAllocationSchema.index({ month: 1, year: 1, is_locked: 1, is_deleted: 1 });
...
// Get next SR number
VerismaDailyAllocationSchema.statics.getNextSrNo = async function(resourceEmail, date) {
...
// Check if Request ID already has "New Request" for Verisma
// Verisma uses "Duplicate" for subsequent entries (unlike MRO which uses "Follow up")
VerismaDailyAllocationSchema.statics.checkRequestIdExists = async function(requestId, clientName = 'Verisma') {
  ...
  return {
    ...
    suggestedType: existing ? 'Duplicate' : 'New Request' // Verisma uses "Duplicate" for subsequent
  };
};

// Get allocations for today
VerismaDailyAllocationSchema.statics.getTodaysAllocations = async function(resourceEmail, date) {
  const startOfDay = new Date(date);
  startOfDay.setHours(0, 0, 0, 0);
  const endOfDay = new Date(date);
  endOfDay.setHours(23, 59, 59, 999);
...
VerismaDailyAllocationSchema.statics.getPreviousLoggedCases = async function(resourceEmail, filters = {}) {
...
VerismaDailyAllocationSchema.statics.editWithHistory = async function(
  allocationId, 
  updates, 
  editorInfo, 
  changeReason, 
...
// Request deletion
VerismaDailyAllocationSchema.statics.requestDeletion = async function(
  allocationId,
  requesterInfo,
  deleteReason
) {
...
VerismaDailyAllocationSchema.statics.reviewDeleteRequest = async function(
  allocationId,
  adminInfo,
  action,
  comment = '',
...
// Get late logs
VerismaDailyAllocationSchema.statics.getLateLogs = async function(filters = {}) {
  const query = {
    is_late_log: true,
    is_deleted: { $ne: true }
  };
...
VerismaDailyAllocationSchema.statics.getPendingDeleteRequests = async function() {
  return this.find({
